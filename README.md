...
- **💾 Persistent Storage**
  ...
  1. Uses SQLite for durable, local data storage

  2. Automatically initializes database schema on startup

- **🗂 Structured Expense Categories**

  1. Exposes predefined categories and subcategories via MCP resources

  2. Categories stored in JSON for easy extensibility

- **🤖 LLM-Friendly Design**

  1. Deterministic tool interfaces

  2. Structured JSON responses

  3. Clean separation between tools (actions) and resources (data)
